...
class GradioUI:
    ...
    def launch(self, **kwargs):
        ...
        with gr.Blocks(fill_height=True) as demo:
            ...
            if self.file_upload_folder is not None:
                upload_file = gr.File(label="Subir archivo")
                upload_status = gr.Textbox(
                    label="Estado de carga", interactive=False, visible=False
                )
                upload_file.change(
                    self.upload_file,
            ...
            text_input = gr.Textbox(
                lines=1,
                label="Tu solicitud",
                placeholder="Escribe tu pregunta aquí y presiona Enter o el botón",
            )
            text_input.submit(
